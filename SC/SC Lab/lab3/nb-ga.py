...
def cross(u,v,l):   #here l is the number of attributes in the dataset
...
#to call only naive bayes
print "Naive bayes accuracy", nb(rows,cols,dataset)
...
for it in range(10):
	...
	for j in range(size):   #probability of fitness of each chromosome
		prob[j] =(fitness[j])/f
	...
	s_x = sorted(prob.items(), key=operator.itemgetter(1))
	for i in range(size):
    		ra = (random.random())
    		for j in range(size):
			if(s_x[j]>ra):
...
print "GA-NB accuracy",max(fitness.values())
